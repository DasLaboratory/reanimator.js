...
async function makelive(serverConfig, path, mode) {
	...
	if (pathType === '.zip') {
		...
		console.log('    📁  /galvanism-for-noobs/en');
		console.log('    📦  das.laboratory-galvanism.for.noobs-en-20210906-web.zip\n\n');
		// console.log(' MODE:\n');
		// console.log('    🎯  ' + mode + '\n\n');
	...
	return await ssh.connect(serverConfig).then(async function () {
		return await ssh
			...
			.then(function (result) {
				...
				if (foundTheLinks) {
					console.log(' LINKS:\n');
					console.log('    🕹️   https://daslaboratory.com/galvanism-for-noobs/en/20210827');
					console.log(
						'    💾  ...or-noobs/en/20210827/das.laboratory-galvanism.for.noobs-en-20210906-web.zip'
					);
